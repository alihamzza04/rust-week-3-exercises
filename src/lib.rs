...
impl TransactionInput {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        res.extend_from_slice(&self.script_sig.to_bytes());
        res.extend_from_slice(&self.sequence.to_le_bytes());
        res
    }
...
impl BitcoinTransaction {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        res.extend_from_slice(&CompactSize::new(self.inputs.len() as u64).to_bytes());
        for input in &self.inputs {
            res.extend_from_slice(&input.to_bytes());
        }
        res.extend_from_slice(&self.lock_time.to_le_bytes());
        res
    }
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        let (count_compact, c1) = CompactSize::from_bytes(&bytes[offset..])?;
        let input_count = count_compact.value as usize;
        offset += c1;

        let mut inputs = Vec::with_capacity(input_count);
        for _ in 0..input_count {
            let (input, c2) = TransactionInput::from_bytes(&bytes[offset..])?;
            inputs.push(input);
            offset += c2;
        }
